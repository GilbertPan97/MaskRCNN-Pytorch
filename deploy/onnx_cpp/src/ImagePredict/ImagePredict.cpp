...
ImagePredict::ImagePredict(bool gpu){
	...
	// }
	// cout << "INFO: Succeed initing ImagePredict";
}
...
bool ImagePredict::PredictAction(cv::Mat& inputImg, float score_thresh){ 
...
cv::Mat ImagePredict::ShowPredictMask(cv::Mat& inputImg, float scoreThreshold){
...
void ImagePredict::softNMSBoxes_filter(float score_threshold, float nms_threshold){
	// data transfer
...
std::vector<cv::Mat> ImagePredict::GetPredictMask()
{
	return masks_;
}

std::vector<int> ImagePredict::GetPredictLabel()
{
	return labels_;
}

std::vector<float> ImagePredict::GetPredictScore()
{
	return scores_;
}
...
cv::Scalar ImagePredict::hsv_to_rgb(std::vector<float> hsv){
...
void ImagePredict::drawDashRect(cv::Mat& img, cv::Point p1, cv::Point p2, 
...
std::vector<cv::Scalar> ImagePredict::random_colors(int nbr, bool bright){
...
Ort::Value ImagePredict::create_tensor(const cv::Mat &mat,
